      ...
      <div className="relative flex-1 flex flex-col space-y-3 p-4">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="relative shrink-0">
            ...
            <div className={`absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 sm:w-4 sm:h-4 rounded-full border-2 border-slate-800 ${
              supplier.is_active ? 'bg-emerald-500' : 'bg-slate-500'
            }`} />
          </div>

          {/* Supplier Info */}
          <div className="flex-1 min-w-0">
            <h3 className="text-white font-semibold text-sm sm:text-base wrap-break-word leading-snug group-hover:text-indigo-300 transition-colors">
              {supplier.name}
            </h3>
            {supplier.phone && (
        ...
        <div className="space-y-2 pt-1">
          ...
          {supplier.address ? (
            <div className="flex justify-center gap-2">
              <span className="text-xs sm:text-sm text-slate-300 line-clamp-3">
                {supplier.address}
              </span>
            </div>
          ) : (
          ...
          <div className="flex justify-center gap-2">
            <span className="text-xs sm:text-sm text-slate-400">
              <span className="font-semibold text-indigo-400">
                {supplier.transaction_details_count || 0}
              </span>{' '}
              transaksi
            </span>
          </div>
        ...
        <div className="pt-2 mt-auto border-t border-slate-700/50">
          ...
          <div className="flex flex-col sm:flex-row gap-2 mb-2">
            ...
            >
              {supplier.is_active ? <XCircle className="w-3.5 h-3.5" /> : <Check className="w-3.5 h-3.5" />}
              <span>{supplier.is_active ? 'Nonaktif' : 'Aktif'}</span>
            </button>
