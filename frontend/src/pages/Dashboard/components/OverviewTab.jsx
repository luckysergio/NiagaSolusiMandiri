...
const OverviewTab = ({ stats, lastUpdate, user, onTabChange }) => {
  ...
  const { data: chartData, isLoading: isChartLoading } = useDashboard().useTransactionChart(chartPeriod);
  const { data: topProducts, isLoading: isTopProductsLoading } = useDashboard().useTopProducts(5);
  const { data: recentTransactions, isLoading: isRecentTxLoading } = useDashboard().useRecentTransactions(5);
        ...
        <div className="relative flex flex-col h-full">
          <div className="flex items-start justify-between mb-3">
            ...
            {trendValue !== undefined && (
              <div className={`flex items-center gap-1 text-xs font-semibold px-2 py-1 rounded-full bg-slate-900/50 border border-slate-700/50 ${isPositive ? 'text-emerald-400' : 'text-red-400'}`}>
          ...
          <div className="flex-1">
            ...
            <p className="text-2xl font-bold text-white mt-2 group-hover:scale-105 transition-transform origin-left tracking-tight">
    ...
    <div className="space-y-6 animate-fadeIn">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
            ...
            <div className="flex items-center gap-1 bg-slate-900/50 p-1 rounded-xl border border-slate-700/50">
              <button
                ...
                className={`px-4 py-1.5 rounded-lg text-xs font-semibold transition-all duration-200 ${chartPeriod === 'weekly' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-slate-800'}`}
              >
                Mingguan
              </button>
              <button
                onClick={() => setChartPeriod('monthly')}
                className={`px-4 py-1.5 rounded-lg text-xs font-semibold transition-all duration-200 ${chartPeriod === 'monthly' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-slate-800'}`}
              >
                Bulanan
              </button>
            </div>
          ...
          {isChartLoading || areaChartData.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={300}>
              <AreaChart data={areaChartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <YAxis stroke="#94a3b8" tick={{fontSize: 11}} axisLine={false} tickLine={false} tickFormatter={(value) => `Rp ${(value/1000000).toFixed(0)}M`} />
                <Tooltip 
        ...
        <Card variant="elevated" className="p-5">
          ...
          <div className="space-y-3">
            {isTopProductsLoading ? (
              ...
              <div key={product.product_id} className="flex items-center justify-between p-3 bg-slate-700/30 hover:bg-slate-700/50 rounded-lg border border-slate-600/30 transition-colors">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-white font-medium text-sm line-clamp-1">{product.product_name}</p>
                    <p className="text-slate-400 text-xs">{product.total_qty} {product.unit} terjual</p>
                  </div>
                </div>
                <p className="text-emerald-400 font-semibold text-sm whitespace-nowrap">{formatRupiah(product.total_revenue)}</p>
              </div>
            )) : (
        ...
        <Card variant="elevated" className="p-5">
          ...
          <div className="space-y-3">
            {isRecentTxLoading ? (
              ...
              <div key={tx.id} className="flex items-center justify-between p-3 bg-slate-700/30 hover:bg-slate-700/50 rounded-lg border border-slate-600/30 transition-colors">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-indigo-500/20 flex items-center justify-center shrink-0">
                    <ShoppingCart className="w-5 h-5 text-indigo-400" />
                  </div>
                  <div className="min-w-0">
                    <p className="text-white font-medium text-sm truncate">{tx.invoice}</p>
                    <p className="text-slate-400 text-xs truncate">{tx.customer_name}</p>
                  </div>
                </div>
      ...
      <Card variant="glass" className="p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="flex items-center gap-3 p-3 bg-slate-900/40 rounded-xl border border-slate-700/30">
