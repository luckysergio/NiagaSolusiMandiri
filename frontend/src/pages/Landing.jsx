...
export default function Landing() {
  ...
  // ✅ OPTIMASI AOS: Inisialisasi sekali dan refresh setelah load untuk mencegah layout shift
  useEffect(() => {
    AOS.init({
      duration: 800,
      once: true,
    ...
    return (
      <div key={service.id} data-aos="fade-up" data-aos-delay={idx * 100} className="group bg-slate-900 rounded-2xl shadow-lg hover:shadow-2xl hover:shadow-indigo-500/10 transition-all duration-500 overflow-hidden border border-slate-800 hover:border-indigo-500/50 hover:-translate-y-2 relative">
        ...
        <div className="relative h-56 overflow-hidden bg-slate-800">
          ...
          {hasMultipleImages && (
            <>
              <button onClick={(e) => { e.stopPropagation(); prevServiceImage(service.id, service.images); }} className="absolute left-3 top-1/2 -translate-y-1/2 p-2 bg-black/50 hover:bg-indigo-600 rounded-full transition-all duration-300 text-white backdrop-blur-sm opacity-0 group-hover:opacity-100" aria-label="Previous image">
                <ChevronLeft className="w-5 h-5" />
              </button>
              <button onClick={(e) => { e.stopPropagation(); nextServiceImage(service.id, service.images); }} className="absolute right-3 top-1/2 -translate-y-1/2 p-2 bg-black/50 hover:bg-indigo-600 rounded-full transition-all duration-300 text-white backdrop-blur-sm opacity-0 group-hover:opacity-100" aria-label="Next image">
                <ChevronRight className="w-5 h-5" />
              </button>
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5">
                {service.images.map((_, imgIdx) => (
                  <button key={imgIdx} onClick={(e) => { e.stopPropagation(); setServiceImageIndex(prev => ({ ...prev, [service.id]: imgIdx })); }} className={`transition-all duration-300 rounded-full ${imgIdx === currentIndex ? 'w-6 h-1.5 bg-white' : 'w-1.5 h-1.5 bg-white/40 hover:bg-white/70'}`} aria-label={`Go to image ${imgIdx + 1}`} />
                ))}
              </div>
            </>
          )}
  ...
  return (
    <div className="overflow-x-hidden">
      <Layout>
        {/* ✅ OPTIMASI SEO: Domain dan deskripsi disesuaikan dengan target keyword */}
        <SEO 
                    ...
                    <div className="relative bg-slate-900/60 backdrop-blur-xl rounded-3xl overflow-hidden shadow-2xl border border-slate-700/50 p-8">
                      <div className="aspect-square bg-linear-to-br from-slate-800 to-slate-900 rounded-2xl flex items-center justify-center p-8 border border-slate-700/50">
                        <div className="text-center">
                          {/* ✅ OPTIMASI SEO: Menambahkan width/height untuk mencegah Cumulative Layout Shift (CLS) */}
                          <div className="w-40 h-40 rounded-2xl bg-white shadow-xl flex items-center justify-center mx-auto mb-6 p-4 border border-slate-200 hover:scale-105 transition-transform duration-500">
                            {logoNsm ? (
                              <img src={logoNsm} alt="Logo Niaga Solusi Mandiri" className="w-full h-full object-contain" width="160" height="160" />
                            ) : (
                              <span className="text-5xl font-bold text-indigo-600">NSM</span>
                            )}
                          </div>
            ...
            <div className="max-w-7xl mx-auto">
              ...
              <div className="relative max-w-4xl mx-auto">
                ...
                <div className="flex items-center justify-center gap-4 mt-8">
                  <button onClick={() => setCurrentTestimonial((prev) => (prev === 0 ? testimonials.length - 1 : prev - 1))} className="p-3 rounded-full bg-slate-800 hover:bg-indigo-600 text-white transition-all duration-300 border border-slate-700 hover:border-indigo-500" aria-label="Previous testimonial">
                    <ChevronLeft className="w-5 h-5" />
                  </button>
                  <div className="flex gap-2">
                    {testimonials.map((_, index) => (
                      <button key={index} onClick={() => setCurrentTestimonial(index)} className={`transition-all duration-300 rounded-full ${index === currentTestimonial ? 'w-8 h-2 bg-indigo-500' : 'w-2 h-2 bg-slate-700 hover:bg-slate-600'}`} aria-label={`Go to testimonial ${index + 1}`} />
                    ))}
                  </div>
                  <button onClick={() => setCurrentTestimonial((prev) => (prev + 1) % testimonials.length)} className="p-3 rounded-full bg-slate-800 hover:bg-indigo-600 text-white transition-all duration-300 border border-slate-700 hover:border-indigo-500" aria-label="Next testimonial">
                    <ChevronRight className="w-5 h-5" />
                  </button>
                </div>
              </div>
              ...
              <div className="space-y-4">
                {faqData.map((faq, idx) => (
                  <div key={idx} data-aos="fade-up" data-aos-delay={idx * 100} className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden hover:border-indigo-500/30 transition-all duration-300">
                    <button onClick={() => toggleFaq(idx)} className="w-full flex items-center justify-between p-6 text-left hover:bg-slate-800/50 transition duration-300" aria-expanded={openFaq === idx}>
                ...
                <div data-aos="fade-left" className="bg-slate-900 rounded-3xl shadow-2xl p-8 border border-slate-800">
                  ...
                  <form className="space-y-5" onSubmit={handleContactSubmit}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                      <div>
                        <label htmlFor="phone" className="block text-sm font-medium text-slate-300 mb-2">No. Telepon / WA</label>
                        <input 
                          id="phone"
                          type="tel" 
                          name="phone"
                      ...
                      <div>
                        <label htmlFor="service" className="block text-sm font-medium text-slate-300 mb-2">Jenis Kebutuhan</label>
                        <select 
                          id="service"
                          name="service"
                    ...
                    <div>
                      <label htmlFor="message" className="block text-sm font-medium text-slate-300 mb-2">Detail Kebutuhan</label>
                      <textarea 
                        id="message"
                        rows="4" 
                        name="message"
