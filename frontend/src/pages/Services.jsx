...
export default function Services() {
  ...
  // ✅ DATA FILOSOFI 3 HEMAT (Ditekankan secara visual untuk Layanan Terpadu)
  const tigaHemat = [
    {
      icon: Timer,
      title: "Hemat Waktu",
        ...
        <div className="absolute inset-0 z-0">
          {heroImages.map((img, index) => (
            <div key={index} className={`absolute inset-0 transition-opacity duration-1000 ease-in-out ${index === currentImageIndex ? 'opacity-100' : 'opacity-0'}`}>
              ...
              <img src={img} alt={`Hero ${index + 1}`} className="w-full h-full object-cover scale-105" loading="eager" />
            </div>
          ))}
        </div>
