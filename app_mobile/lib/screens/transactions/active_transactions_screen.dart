...
class ActiveTransactionsScreen extends StatefulWidget {
  ...
  @override
  State<ActiveTransactionsScreen> createState() =>
      _ActiveTransactionsScreenState();
}
...
class _ActiveTransactionsScreenState extends State<ActiveTransactionsScreen> {
  ...
  String get _currentStatus => 'dipesan,dikerjakan';
  ...
  Future<void> _loadTransactions({bool showLoading = true}) async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        final allTransactions = result['transactions'] as List<Transaction>;
  ...
  // ✅ PERBAIKAN 2: Menambahkan dialog konfirmasi sebelum ubah status
  Future<void> _handleChangeStatus(
      Transaction transaction, String newStatus) async {
    final confirmed = await _showConfirmation(
      'Ubah Status',
      'Apakah Anda yakin ingin mengubah status transaksi "${transaction.invoice}" menjadi "$newStatus"?',
    );
    ...
    try {
      await TransactionService.changeStatus(transaction.id, newStatus);
      if (!mounted) return;
      _showSuccess('Berhasil', 'Status transaksi berhasil diubah');
  ...
  // ✅ PERBAIKAN 1: Fungsi untuk membuka form dan auto-refresh jika berhasil
  Future<void> _openCreateForm() async {
    final result = await context.push('/transactions/create');
            ...
            if (_canCreate)
              Positioned(
                ...
                child: FloatingActionButton(
                  // ✅ PERBAIKAN 1: Menggunakan fungsi _openCreateForm untuk auto-refresh
...
// ✅ PERBAIKAN 3: Mengubah menjadi StatefulWidget untuk mengelola state loading tombol invoice per-kartu
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        ...
        Padding(
          ...
          child: Row(children: [
            if (statusConfig['next'] != null) ...[
              Expanded(
                  child: _ActionBtn(
                      icon: Icons.check_circle_rounded,
                      label: statusConfig['nextLabel'],
                      color: const Color(0xFF3B82F6),
                      onTap: widget.isMutating
                          ? null
                          : () => widget.onChangeStatus(
                              widget.transaction, statusConfig['next']))),
              const SizedBox(width: 4),
            ],
            if (widget.canEdit) ...[
              Expanded(
